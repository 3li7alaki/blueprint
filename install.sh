#!/bin/sh
# blueprint installer: slim, no runtime deps, no root, idempotent (also acts as updater).
#   curl -fsSL https://raw.githubusercontent.com/3li7alaki/blueprint/main/install.sh | sh
#   curl -fsSL .../install.sh | sh -s -- --uninstall
#
# blueprint is a single self-contained Go binary, distributed as a rolling `latest` release
...
# onto your PATH at ~/.local/bin/blueprint. Re-run it any time to update in place.
set -eu

TOOL="blueprint"
REPO="3li7alaki/blueprint"
...
case "$OS" in
  linux|darwin) ;;
  *) err "unsupported OS: $OS (blueprint ships linux + darwin)" ;;
esac
ARCH=$(uname -m)
case "$ARCH" in
  x86_64|amd64)  ARCH="amd64" ;;
  aarch64|arm64) ARCH="arm64" ;;
  *) err "unsupported architecture: $ARCH (blueprint ships amd64 + arm64)" ;;
esac
TARGET="${OS}-${ARCH}"
...
if command -v "$TOOL" >/dev/null 2>&1; then
  say "installed $TOOL $INSTALLED. Run: blueprint --help"
else
  say "installed $TOOL $INSTALLED to $BIN_DIR/$TOOL"
